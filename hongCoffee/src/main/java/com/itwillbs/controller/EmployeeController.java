...
//	}
	...
	public String order(HttpServletRequest request, Model model,PageDTO pageDTO) {
		...
		// 페이징 작업
		// 전체 글개수 구하기  int 리턴할형 count = getStoreCount(pageDTO) 검색어 포함
		int count = employeeService.getOrderCount(orderDTO);
		// 한 화면에 보여줄 페이지 개수 설정
		int pageBlock = 10;
		// 한 화면에 보여줄 시작페이지 구하기
		// 1~10 => 1, 11~20 => 11,..
...
//		}
	...
	public String shipment(HttpServletRequest request, Model model, PageDTO pageDTO) {
		...
		int count = employeeService.getShipmentCount(shipmentDTO);
		// 한 화면에 보여줄 페이지 개수 설정
		int pageBlock = 10;
		// 한 화면에 보여줄 시작페이지 구하기
		// 1~10 => 1, 11~20 => 11,..
...
//		}
		...
		try {
			shipmentDTO.setReceived_not(received_sNot != null ? Integer.parseInt(received_sNot) : 100);
		} catch (NumberFormatException e) {
			e.printStackTrace();
		}	
		...
		try {
			shipmentDTO.setPay(sPay != null ? Integer.parseInt(sPay) : 100);
		} catch (NumberFormatException e) {
			e.printStackTrace();
		}
		...
		if (name == "" && item_name == "" && sh_minTime == "" && sh_maxTime == "" && received_sNot == null && sPay == null) {
	...
	public String emp(HttpServletRequest request, Model model,PageDTO pageDTO) {
		...
		// 페이징 작업
				// 전체 글개수 구하기  int 리턴할형 count = getStoreCount(pageDTO) 검색어 포함
				int count = employeeService.getEmployeeCount(employeeDTO);
				// 한 화면에 보여줄 페이지 개수 설정
				int pageBlock = 10;
				// 한 화면에 보여줄 시작페이지 구하기
				// 1~10 => 1, 11~20 => 11,..
